 ...
 * proxyflow Web SDK
 *
 * 将浏览器的 fetch 请求转发到 proxyflow 服务，实现请求拦截、录制和 Mock 功能。
 *
 * 基本用法（script 标签）:
 *   <script src="proxyflow-web-sdk.js"></script>
 *   <script>
 *     proxyflow.init({ serverUrl: 'http://localhost:9000', sessionId: 'xxx' });
 *     proxyflow.patch(); // 自动拦截全局 fetch
 *   </script>
 *
 * 基本用法（ESM / npm）:
 *   import proxyflow from 'proxyflow-web';
 *   proxyflow.init({ serverUrl: 'http://localhost:9000', sessionId: 'xxx', autoPatch: true });
 */
...
(function (global, factory) {
  ...
  function relayFetch(input, init) {
    ...
    if (!_originalFetch) {
      warn('No original fetch found. Call proxyflow.init() before making requests.');
      return Promise.reject(new Error('proxyflow not initialized properly'));
    }
    ...
    // FormData / Blob / ReadableStream 无法序列化为 JSON，直接走原始 fetch
    if (
      merged.body instanceof FormData ||
      merged.body instanceof Blob ||
      (typeof ReadableStream !== 'undefined' && merged.body instanceof ReadableStream)
    ) {
      log('Non-serializable body detected, falling back to direct request for', url);
    ...
    var timeoutId = controller
      ? setTimeout(function () { controller.abort(); }, _config.timeout)
      : null;
    ...
    return _originalFetch.call(globalThis, relayUrl, fetchOptions).then(function (relayResponse) {
  ...
  var proxyflow = {
     ...
     *
     * @param {object} config
     *   serverUrl  {string}  proxyflow 后端地址，如 "http://localhost:9000"（必填）
